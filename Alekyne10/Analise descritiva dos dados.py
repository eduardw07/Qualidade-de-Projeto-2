...
merge_by_project = df.groupby('project_name')['is_merge'].mean()
print(" ")
...
contagem_merges_por_classe = df[df['is_merge'] == 1].groupby('Engenheirado').size()
...
tfidf_matrix = tfidf.fit_transform(df['project_name'])
...
X = df.drop(columns=['project_name', 'Engenheirado'])
y = df['Engenheirado']
...
df_resampled = df_resampled.join(df[['project_name']].reset_index(drop=True), how='left')
